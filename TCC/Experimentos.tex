...
A comparação das quatro estratégias de controle baseia-se em métricas quantitativas calculadas sobre as séries temporais de erro de junta $e(t) = q_d(t) - q(t)$ e torque de controle $\tau(t)$ armazenadas pelo método \texttt{RobotSimulator.run()}, conforme descrito na Seção~\ref{sec:arq_simulador} do Capítulo~\ref{cap:simulacao}. Quatro métricas são utilizadas para a avaliação quantitativa: o Erro Quadrático Médio por junta (RMSE), o Erro de Pico, o Torque RMS e o Índice de \textit{Chattering}, definidas a seguir.

O Erro Quadrático Médio por Junta (RMSE), apresentado na Equação~\eqref{eq:rmse}, pondera erros grandes mais pesadamente que erros pequenos, sendo sensível tanto à fase de transiente quanto ao regime permanente \cite{astrom1995theory}:
\begin{equation}
    \varepsilon_{\text{rms},i} = \sqrt{\frac{1}{T_f}\int_0^{T_f} e_i(t)^2\,dt},
    \label{eq:rmse}
\end{equation}
onde:
\begin{itemize}
    \item $\varepsilon_{\text{rms},i}$ é o erro quadrático médio (RMSE) da junta $i$ ao longo da trajetória;
    \item $e_i(t) = q_{d,i}(t) - q_i(t)$ é o erro de posição de junta no instante $t$;
    \item $T_f$ é a duração total da trajetória.
\end{itemize}

O Erro de Pico, definido na Equação~\eqref{eq:peak_error}, captura o pior desvio instantâneo ao longo da trajetória, sendo relevante para restrições de segurança em espaços confinados \cite{siciliano2009robotics}:
\begin{equation}
    \varepsilon_{\text{pk},i} = \max_{t \in [0, T_f]} |e_i(t)|,
    \label{eq:peak_error}
\end{equation}
onde $\varepsilon_{\text{pk},i}$ é o erro de pico (máximo absoluto) da junta $i$ e $e_i(t)$ é o erro instantâneo conforme definido em~\eqref{eq:rmse}.

O Torque RMS, expresso pela Equação~\eqref{eq:torque_rms}, é uma métrica indireta do consumo energético da junta $i$ e indicador do esforço do atuador \cite{spong2020robot, craig1989introduction}:
\begin{equation}
    \tau_{\text{rms},i} = \sqrt{\frac{1}{T_f}\int_0^{T_f} \tau_i(t)^2\,dt},
    \label{eq:torque_rms}
\end{equation}
onde $\tau_i(t)$ é o torque aplicado à junta $i$ no instante $t$ e $T_f$ é a duração total da trajetória.

O Índice de \textit{Chattering}, dado pela Equação~\eqref{eq:chattering}, é a integral do valor absoluto da derivada do torque, penalizando variações rápidas características do \textit{chattering} de controle por modo deslizante \cite{edwards1998sliding, utkin2017sliding}:
\begin{equation}
    \chi_i = \frac{1}{T_f}\int_0^{T_f} |\dot{\tau}_i(t)|\,dt,
    \label{eq:chattering}
\end{equation}
onde $\chi_i$ é o índice de \textit{chattering} da junta $i$ e $\dot{\tau}_i(t)$ é a derivada temporal do torque aplicado.
...
onde:
\begin{itemize}
    \item $\rho_i$ é o fator adimensional de degradação do desempenho da junta $i$ (\textit{performance degradation ratio});
    \item $\varepsilon_{\text{rms},i}^{(\text{pert})}$ é o RMSE de junta no cenário com perturbações ou incerteza paramétrica introduzidas;
    \item $\varepsilon_{\text{rms},i}^{(\text{nom})}$ é o RMSE de junta no cenário nominal (sem perturbações);
    \item valores de $\rho_i$ próximos de 1 indicam alta robustez (degradação desprezível) e valores elevados ($\rho_i \gg 1$) indicam sensibilidade às incertezas \cite{slotine1991applied, moreno2012strict}.
\end{itemize}
...
\begin{itemize}
    ...
    \item Passo de visualização: $\Delta t_{\text{vis}} = 5 \times 10^{-2}$ s (50 \textit{substeps} por \textit{frame});
...
A condição de estabilidade do integrador simplético, dada pela Equação~\eqref{eq:stability_condition} da Seção~\ref{sec:estabilidade_num} do Capítulo~\ref{cap:simulacao}, é satisfeita com ampla margem para o ganho proporcional $k_p = 100$ utilizado nos experimentos, uma vez que $\Delta t_{\text{phys}} = 10^{-3}$ s é cerca de duzentas vezes inferior ao limite $2/\sqrt{k_p} = 0,2$~s \cite{leimkuhler2004simulating}.
...
\begin{equation}
\begin{aligned}
    \texttt{joint\_config} = \big[\,
        & \underbrace{\texttt{'Dx'},\, \texttt{'Dy'},\, \texttt{'Dz'},\, \texttt{'Rx'},\, \texttt{'Ry'},\, \texttt{'Rz'}}_{\text{veículo base}},\, \\
        & \underbrace{\texttt{'Rz'},\, \texttt{'Ry'},\, \texttt{'Ry'},\, \texttt{'Rz'},\, \texttt{'Rx'},\, \texttt{'Rz'}}_{\text{braço antropomórfico}}\,\big],
\end{aligned}
\label{eq:uvms_config}
\end{equation}
onde:
\begin{itemize}
    \item \texttt{joint\_config} é a lista ordenada que descreve o tipo de cada uma das $N=12$ juntas do UVMS;
    \item \texttt{'Dx', 'Dy', 'Dz'} representam juntas prismáticas ao longo dos eixos $x$, $y$ e $z$, modelando a posição do veículo (\textit{surge}, \textit{sway}, \textit{heave});
    \item \texttt{'Rx', 'Ry', 'Rz'} representam juntas rotacionais em torno dos eixos $x$, $y$ e $z$, modelando a orientação do veículo (\textit{roll}, \textit{pitch}, \textit{yaw}) e as juntas do braço;
    \item as seis primeiras entradas (índices 1--6) modelam o veículo subaquático como uma cadeia de seis juntas seriais que reproduzem o movimento 6-DOF do casco;
    \item as seis últimas entradas (índices 7--12) modelam o braço antropomórfico com ombro (Rz, Ry, Ry), cotovelo (Rz) e punho esférico (Rx, Rz);
    \item os colchetes denotam a estrutura de lista Python (entrada do método \texttt{RobotMathHydro}) \cite{antonelli2014modelling, fossen2011handbook}.
\end{itemize} As juntas do veículo não possuem vetor de elo geométrico; o braço possui vetores de elo conforme a Tabela~\ref{tab:uvms_mecanico}.
...
onde:
\begin{itemize}
    \item $ma_{u,i}, ma_{v,i}, ma_{w,i}$ são os coeficientes de massa adicionada translacionais (em kg) do elo $i$ nas direções $u$ (\textit{surge}), $v$ (\textit{sway}) e $w$ (\textit{heave}) do referencial local;
    \item $ma_{p,i}, ma_{q,i}, ma_{r,i}$ são os coeficientes de massa adicionada rotacionais (em kg·m$^2$) do elo $i$ em torno dos eixos $p$ (\textit{roll}), $q$ (\textit{pitch}) e $r$ (\textit{yaw});
    \item $\rho$ é a massa específica do fluido (água do mar: $\rho \approx 1025$ kg/m$^3$);
    \item $\text{vol}_i$ é o volume submerso do elo $i$ (em m$^3$);
    \item $L_i$ é o comprimento do elo $i$ (em m);
    \item o fator $\frac{1}{2}\rho\,\text{vol}_i$ corresponde ao coeficiente de massa adicionada de uma esfera de mesmo volume (movimento translacional);
    \item o fator $\frac{1}{12}\rho L_i^2\,\text{vol}_i$ corresponde ao momento de massa adicionada de um corpo cilíndrico esbelto equivalente \cite{newman1977marine, fossen2011handbook}.
\end{itemize}
...
\hline
CTC-PID & $k_p = 25$ rad$^2$/s$^2$, $\zeta = 1{,}0$, $k_i = 0$ (ou 2 para CTC-PID), \textit{anti-windup} = 10 \\
...
Apresentam-se, na Tabela~\ref{tab:uvms_resultados}, as métricas de desempenho para os quatro controladores no UVMS de 12 GDL com modelo nominal, ou seja, os parâmetros de massa adicionada utilizados pelo controlador coincidem com os da planta simulada. Esses resultados são sintetizados visualmente na Figura~\ref{fig:uvms_comparativo}, com o erro e o torque de cada junta ao longo do tempo para os quatro controladores, além das métricas agregadas (RMSE médio, energia, torque máximo e tempo de cómputo).
...
\begin{tabular}{p{2.6cm} p{1.9cm} p{1.4cm} p{2.0cm} p{1.6cm} p{1.3cm}}
\hline
...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{figuras/Experimentos/UVMS_COMPARATIVO.png}
...
\textbf{CTC-PID (Torque Computado)}: a perturbação $d(t)$ entra na dinâmica de erro linear como forçamento externo. A rejeição depende inteiramente do ganho integral $K_I$, que elimina perturbações constantes mas produz \textit{overshoot} e ressonância para perturbações variáveis no tempo. Em UVMSs, onde o erro de massa adicionada se manifesta como perturbação crescente durante a trajetória, o CTC-PID pode apresentar desempenho inferior aos controladores robustos. Ilustra-se, na Figura~\ref{fig:ctc_graf}, o perfil típico: torque com pico inicial para iniciar o movimento, seguido de estabilização \cite{ogata2010modern, astrom1995theory, fossen2011handbook}.
...
\textbf{LADRC}: a perturbação é estimada explicitamente pelo ESO como terceiro estado estendido $z_3 \approx f_i$, e cancelada na lei de controle pela subtração $-z_3/b_0$. Observa-se, na Figura~\ref{fig:adrc_graf}, o perfil de torque do ADRC: pico inicial moderado e estabilização suave, com boa eficiência energética. A qualidade do cancelamento é limitada pelo \textit{lag} dinâmico do observador (erro de estimação $\propto \delta_i/\omega_o$), mas não pela amplitude da perturbação em si, propriedade que explica a robustez superior e a capacidade do ESO de \textit{linearizar ativamente} a planta \cite{gao2003scaling, zheng2010practical, huang2014active, han2009pid, han1995class}.
...
O experimento com o UVMS de 12 GDL resultou em tempo de computação de aproximadamente 48 s para 6 s de trajetória simulada (cerca de 8 s de tempo real por segundo simulado), indicando que a simulação é viável em \textit{hardware} convencional. O executor persistente (\texttt{warm\_up\_workers()}) e a compilação com CSE são essenciais para manter o custo de avaliação de $M$, $C$ e $G$ dentro de limites que permitam a integração numérica estável \cite{beazley2010understanding, leu1986automated}.
...
\begin{itemize}
    ...
    \item \textbf{\textit{Trade-off} torque máximo}: o Super-Twisting exige torque máximo aproximadamente 4 vezes maior ($\approx 79$ N·m) que os demais controladores ($\approx 21$ N·m), impactando o dimensionamento dos atuadores \cite{levant1993sliding, shtessel2014sliding, utkin2017sliding}.

    \item \textbf{Desempenho computacional}: o tempo de simulação de $\approx 48$ s para 6 s de trajetória indica que a implementação dos quatro controladores é viável para o UVMS de 12 GDL em \textit{hardware} convencional, com CSE e executor persistente \cite{meurer2017sympy, leu1986automated, beazley2010understanding}.
